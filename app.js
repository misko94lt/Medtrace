...
function IECReportForm({ initial, assetId: propAssetId, assets, customers, onSave, onClose }) {
    const getMeasures = React.useCallback((norm, cls, patientType) => {
        ...
        // ── IEC 62353 — limiti per tipo parte applicata ──────────────
        const pt = patientType || "BF";
        // µA — c.n. = condizione normale, c.g. = guasto singolo
        const patLim = {
        ...
        if (cls === "III")
            return [
                // No PE, no dispersione rete (tensione sicura)
                ...
                // Nota: in Cl.III non si misura dispersione verso terra (circuito SELV)
            ];
        // ── Classe II — doppio isolamento (no PE) ─────────────────────
        if (cls === "II")
            return [
                // No resistenza PE (apparecchio senza terra di protezione)
...
const HELP_SECTIONS = [
    ...
    },
    {
        ...
        steps: [
            ...
            "Dalla scheda puoi aprire + Job, Verifica Sicurezza Elettrica o  Verifica Funzionale direttamente per quell'apparecchio.",
    ...
    },
    {
        ...
        steps: [
            ...
            "Quando una Verifica Sicurezza Elettrica o Funzionale viene salvata, crea automaticamente un job 'chiuso' collegato.",
        ]
    },
    {
        icon: "›", title: "Verifiche Sicurezza Elettrica (Sicurezza Elettrica)", color: "#2DD4BF",
        steps: [
    ...
    },
    {
        ...
        steps: [
            ...
            "Si popola automaticamente quando salvi una Verifica Sicurezza Elettrica (nextService = +1 anno stesso mese).",
    ...
    },
    {
        ...
        steps: [
            ...
            "Clicca  Copia tutto, poi apri Blocco Note, incolla e salva come medtrace-backup.json.",
...
function AssetDetailModal({ asset, jobs, parts, iecReports, funcReports, customers, onClose, onEditAsset, onNewJob, onNewIec, onNewFunc, onOpenJob, company, generateJobPDF, generateIECPDF, generateFuncPDF }) {
    ...
    const TABS = [{ id: "overview", label: " Scheda" }, { id: "jobs", label: " Job (" + assetJobs.length + ")" }, { id: "iec", label: "Sic. Elettrica (" + assetIec.length + ")" }, { id: "func", label: "Funzionale (" + assetFunc.length + ")" }];
...
function App() {
    ...
    const delAsset = id => { setAssets(a => a.filter(x => x.id !== id)); showToast("Eliminato", "#ef4444"); };
    const savePart = f => {
        if (modal.data) {
    ...
    const delJob = id => { setJobs(j => j.filter(x => x.id !== id)); showToast("Eliminato", "#ef4444"); };
    ...
    const saveIecReport = f => {
        ...
        if (isNew) {
            ...
            if (!isExtraordinary) {
                ...
                showToast("Verifica Sicurezza Elettrica salvata + Job " + newJobId + " creato · Prossima: " + nextServiceDate);
            }
            else {
                setAssets(as => as.map(a => a.id === f.assetId ? Object.assign(Object.assign({}, a), { lastService: f.date || new Date().toISOString().slice(0, 10) }) : a));
                showToast("Verifica Sicurezza Elettrica straordinaria salvata + Job " + newJobId + " creato (pianificazione invariata)");
            }
        }
        else {
    ...
    const delIecReport = id => { setIecReports(rs => rs.filter(r => r.id !== id)); showToast("Eliminato", "#ef4444"); };
    ...
    const delFuncReport = id => { setFuncReports(rs => rs.filter(r => r.id !== id)); showToast("Eliminato", "#ef4444"); };
            ...
            tab === "assets" && (React.createElement("div", null,
                ...
                assets.length === 0 ? (React.createElement("div", { style: { textAlign: "center", padding: "40px 18px", background: "#141418", borderRadius: 12, border: "1px dashed #2a3040" } },
                    ...
                    React.createElement(Btn, { onClick: () => setModal({ type: "asset", data: null }) }, "Nuovo apparecchio"))) : (React.createElement(ExcelTable, { defaultSort: "name", onRowClick: row => setModal({ type: "assetDetail", data: assets.find(a => a.id === row.id) }), rowBg: row => row.status === "fuori servizio" ? "#ef333308" : row.status === "in manutenzione" ? "#f59e0b08" : "", cols: [
                        ...
                        React.createElement("button", { onClick: () => setModal({ type: "iec", assetId: row.id, data: null }), title: "Verifica Sicurezza Elettrica", style: { background: "#202028", border: "1px solid #2a3040", borderRadius: 5, color: "#5EEAD4", padding: "3px 8px", cursor: "pointer", fontSize: 11 } }, "\u26A1"),
